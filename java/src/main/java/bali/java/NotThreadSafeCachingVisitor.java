...
final class NotThreadSafeCachingVisitor implements MethodVisitor {
    ...
    public Consumer<Output> visitNonNullField(Method m, String prefix) {
        return out -> out
                .nl()
                .ad(prefix).ad(m.getMethodReturnTypeLocalized()).ad(" ").ad(m.getMethodName()).ad(";").nl();
    }
    ...
    public Consumer<Output> visitNullableField(Method m, String prefix) {
        return out -> out
                .nl()
                .ad(prefix).ad("java.util.function.Supplier<").ad(m.getMethodReturnTypeLocalized()).ad("> ").ad(m.getMethodName()).ad(";").nl();
    }

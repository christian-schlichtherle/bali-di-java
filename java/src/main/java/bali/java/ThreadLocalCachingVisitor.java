...
final class ThreadLocalCachingVisitor implements MethodVisitor {
    ...
    public Consumer<Output> visitNonNullField(Method m, String prefix) {
        return out -> out
                .nl()
                .ad(prefix).ad("final java.lang.ThreadLocal<").ad(m.getMethodCacheTypeLocalized()).ad("> ").ad(m.getMethodName()).ad(" = new java.lang.ThreadLocal<>();").nl();
    }
    ...
    public Consumer<Output> visitNullableField(Method m, String prefix) {
        return out -> out
                .nl()
                .ad(prefix).ad("final java.lang.ThreadLocal<java.util.function.Supplier<").ad(m.getMethodCacheTypeLocalized()).ad(">> ").ad(m.getMethodName()).ad(" = new java.lang.ThreadLocal<>();").nl();
    }
